...
export type HoldingSummary = {
    ...
    // EUR metrics: costs at each transaction's historical exchange_rate
    // (fallback: current rate), market value at the current rate
    current_value_eur: number
    total_invested_eur: number
    total_fees_eur: number
    realized_gl_eur: number
    unrealized_gl_eur: number
    unrealized_gl_eur_pct: number
    unrealized_gl_with_fees_eur: number
    unrealized_gl_with_fees_eur_pct: number
    total_gl_eur: number
}
