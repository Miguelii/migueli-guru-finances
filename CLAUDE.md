...
- Use `protectedProcedure` (from `@/_bff/trpc/server`) for authenticated routes — `ctx.user` is guaranteed; pass it into the service. Auth is enforced by the middleware, so **never** map `UnauthenticatedError` / `GetUserError` in the `Match`.
...
- Register controllers in the module's `<module>.router.ts` (exporting `<MODULE>_ROUTER`), and register that router under its namespace in `src/_bff/trpc/router.ts`.
...
- **Invalid session → forced sign-out**: `UNAUTHORIZED` (401) is reserved for an invalid session — never map business errors (e.g. wrong password → `BAD_REQUEST`) to it. `createCaller()` redirects to `GET /api/auth/sign-out` on 401 (via `onError`), and the client `unauthorizedLink` (`src/lib/trpc-unauthorized-link.ts`) hard-navigates there. The route handler (`force-sign-out.service.ts`) signs out locally and deletes all `sb-*` cookies, so it works even when the session is already dead. `runEffect` upgrades PostgREST JWT rejections (`PGRST301`–`PGRST303`) to 401; `getSession` only returns "no user" for invalid-session auth errors (network/5xx stay 500 so users aren't signed out by outages)
...
- **Consumption**: Server Components use `createCaller()` (`src/_bff/trpc/caller.ts`); Client Components use `trpcClient.*.useMutation`/`useQuery` (`src/lib/trpc.ts`; provider `src/providers/trpc-provider.tsx` in `layout.tsx`, `httpBatchLink` → `/api/trpc`)
